...
function countProfit(shoppers) {
    ...
    for (var indexShopper = 0 ; indexShopper < shoppers.length ; indexShopper++ ){
        for(var indexBarang = 0 ; indexBarang < listBarang.length ; indexBarang++ ){            
            if(listBarang[indexBarang][0] === shoppers[indexShopper].product  && result[indexBarang].leftOver >= shoppers[indexShopper].amount ){
                result[indexBarang].leftOver =  result[indexBarang].leftOver - shoppers[indexShopper].amount
                result[indexBarang].shoppers.push(shoppers[indexShopper].name)
                result[indexBarang].totalProfit = result[indexBarang].totalProfit + (listBarang[indexBarang][1] * shoppers[indexShopper].amount )
            }
